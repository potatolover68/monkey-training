...
def analyze_text(text_file: str, bloom_filter: flowers.Flower, language: str) -> None:
    ...
    print("=" * 50)
    print(f"{Fore.MAGENTA}Most {language} Sentence:{Style.RESET_ALL} {most_matching_sentence}")
    print(f"{Fore.MAGENTA}{language} Score:{Style.RESET_ALL} {Fore.YELLOW}{score:.3f}{Style.RESET_ALL}")
    print("=" * 50)

def analyze_text_with_score(text_file: str, bloom_filter: flowers.Flower, language: str) -> tuple[str, float]:
...
for text_file in texts:
    ...
    display_name = text_file.split('/')[-1].replace('.txt', '').replace('_', ' ').title()
    
    # Analyze for both languages
    fr_sentence, fr_score = analyze_text_with_score(text_file, fr, "French")
    eng_sentence, eng_score = analyze_text_with_score(text_file, eng, "English")
...
for result in sorted(results, key=lambda x: x['ratio'], reverse=True):
    ...
    print(f"{Fore.MAGENTA}Most English Sentence:{Style.RESET_ALL}\n  \"{result['eng_sentence']}\"")
    print("-" * 30)
...
print(f"The most French-like text is {Fore.GREEN}{most_french['name']}{Style.RESET_ALL}")
print(f"with a French/English ratio of {Fore.YELLOW}{most_french['ratio']:.3f}{Style.RESET_ALL}")
print("=" * 50)